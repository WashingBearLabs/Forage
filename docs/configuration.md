...
| `search_searxng_timeout_seconds` | float | `10.0` | `10.0` | SearXNG timeout settings seam, validated unconditionally at boot; range 1.0 to 60.0, wrong-typed or out-of-range values raise `SearxngConfigurationError`. The read path still uses a fixed 10.0-second per-socket-operation timeout on the SearXNG call; the configured value becomes a wall-clock budget from provider-bounds US-003 onward. Changing it does not yet change request timing. |
