...
**38 `test_*.py` modules** under `tests/`, flat, one per subject — 41 Python files in all
...
| `tests/test_orchestrator.py` | 357 | End-to-end pipeline drive, search + retrieve paths |
...
| `tests/test_url_validator.py` | 257 | SSRF defense: RFC1918, DNS rebinding, schemes; exact IPv6 network-list values, independent of Python's mapped-address string formatting |
...
| `tests/test_sanitizer_revision.py` | 27 | Revision hashing over `_REVISION_SOURCES` and the `MODEL_ID@revision` model identity |
...
test_mapping:
  ...
  "pipeline/orchestrator.py": ["tests/test_orchestrator.py", "tests/test_search_pipeline_pins.py", "tests/test_promptguard_policy.py", "tests/test_retrieve_admission.py"]
