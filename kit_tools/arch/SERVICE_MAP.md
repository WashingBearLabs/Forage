...
| Attribute | Value |
|-----------|-------|
| **Purpose** | The only search backend for `POST /search`. Every result's title, URL and snippet is run through pipeline stages 1-3 before it is returned. |
| **Client / protocol** | `httpx.AsyncClient(timeout=10.0)` GET `{SEARXNG_URL}/search` with `q`, `format=json`, `pageno=1`, `engines=duckduckgo,brave,startpage,mojeek` (`_SEARXNG_ENGINES` in `pipeline/orchestrator.py`). One request per `/search`; at most `min(num_results*2, 20)` candidates are scanned; `unresponsive_engines` is passed through from the JSON envelope. |
...
| **Failure impact** | `/search` returns **422** `{error, reason, request_id}` with `searxng_error` (non-2xx: `"SearXNG returned HTTP <code>"`; a 429 here means the limiter was turned on) or `searxng_unavailable` (connect refused, DNS, timeout, bad JSON: `"SearXNG not reachable at <SEARXNG_URL>: <exc>"` — the reason echoes the URL and exception text). `/retrieve` and `/extract` are unaffected. `/metrics.search.errors` is keyed by code. |
...
| **Health signal** | `/health.promptguard_loaded` (always honest), `degraded_reasons: ["promptguard_unavailable"]`, `capabilities: {}` (vs `{"search_sanitization": 1}` when loaded). `/metrics.model`: `fetch_failures`, `verify_failures`, `quarantines`, `fetch_in_progress`, `retries_scheduled` — the only way to tell "downloading" (`fetch_in_progress: true`) from "waiting out backoff" (`retries_scheduled > 0`) from "first attempt unfinished" (both zero). |
...
| **SSRF defence** | Schemes `http`/`https` only; hostname `localhost` and suffix `.local` rejected before DNS; private and reserved IPv4 and IPv6 ranges (including IPv4-mapped IPv6, link-local, CGN, multicast, documentation ranges, `0.0.0.0` and `::`); unparseable addresses treated as unsafe. Full range list in `url_validator.py`. |
