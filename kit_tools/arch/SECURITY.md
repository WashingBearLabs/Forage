...
| Operator spend: with `FORAGE_BRAVE_API_KEY` set and `brave` in the chain, every unauthenticated `POST /search` that reaches Brave is one billable call, and Forage enforces no spend or provider-fetch concurrency ceiling. On `[searxng, brave]` the **free peer's failures control paid calls**: an over-bound, undecodable, malformed or timed-out SearXNG body each buys one Brave call. `SEARXNG_URL` defaults to plain HTTP, so an on-path party can force those calls. Network placement is the control; `/metrics` `search.paid_calls` and `search.provider_compressed_body` are the detection floor, and the budget breaker is the consumer's. | ruling 12; `hardening-provider-bounds` US-003; `docs/configuration.md` |
