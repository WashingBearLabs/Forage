...
- [ ] Every new outbound request goes through `validate_url` and, if it follows redirects, through `fetch_url`'s manual, per-hop revalidated path rather than `follow_redirects=True` — except a `SearchProvider` implementation's own call (`pipeline/search_providers/`, e.g. `SearxngProvider`, `BraveApiProvider`), whose endpoint is operator-configured or a fixed constant read only at process start and is exempt from `validate_url` on that basis alone (`SearchProvider` protocol docstring, rule 1); such a provider still never follows redirects, and the results it returns are trusted by nothing until the sanitization loop has scanned them.
