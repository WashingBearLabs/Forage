...
`/extract` error reasons are fixed, content-free strings (`DOCUMENT_FAILURE_REASONS`). `/retrieve` `fetch_error` and `/search` `searxng_unavailable` do interpolate the caught exception; see "Known Limitations and Open Questions". The full error vocabulary is in `kit_tools/arch/patterns/ERROR_HANDLING.md`.
...
- **Exception text on the wire.** `/retrieve` `fetch_error` and `/search` `searxng_unavailable` reasons interpolate `str(exc)` (and, for search, the SearXNG URL) into the response body (`pipeline/orchestrator.py` lines 327 and 696). Whether upstream error text can carry anything sensitive is neither documented nor tested. Redaction would be a wire change under `contract/GOVERNANCE.md`.
