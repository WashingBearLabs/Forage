...
The URL-side guarantee is therefore: **the served `url` is the provider's trimmed raw value modulo canonicalisation** (scheme and host lower-cased, fragment dropped, IPv6 re-bracketed), **at most 2 048 characters, and both its entity-decoded and its once-percent-decoded forms were scanned**. The reason split is deliberate: rules (0)–(3) count under `invalid_url` and rule (4) under `structural_blocked`, each rejection counted exactly once under the first rule that fired. `domain` is computed only after (0)–(3) pass, so it is never derived from a rejected URL. Every (0)–(3) rejection emits one content-free `search_url_rejected rule=<token> provider=<name>` record — the rule token and the provider name, never the URL or its host (invariant 6).
