...
**Stage 3 applies two independent rules, with contiguity shipped off.**
`PromptGuardSettings` is built once in lifespan and passed through the threshold
seam to all routes (both upload entry points included). The max rule uses `>`;
the run rule uses `>=` over 2–8 adjacent windows, default `0` disabled and
absolute threshold `0.5`. `PromptGuardMetricsSink` carries the shared counter:
retrieve/upload increment before stage-4 structuring, search before omission.
Stage 3 emits one content-free WARNING per contiguity/both verdict. Search
coverage is content-dependent; equal character lengths can yield different
window counts. The quarantine source is byte-unchanged.

This is the thirty-ninth rotation (`85394a95…` → `b641e6a5…`):
`stage3_promptguard.py`, `orchestrator.py`, `contract.py`, plus windows then
threshold as new ASCII hash inputs after the max threshold. Every file/input
was reversed independently read-only against clean `967748d`; the all-reverted
control reproduces `85394a95…` under default and shipped config. Other six
hashed sources are unchanged. All cache keys invalidate even with the run rule
