...
| `search_searxng_timeout_seconds` | `10.0` | `10.0` | float, 1.0 to 60.0 (wrong-typed or out-of-range refuses boot with `SearxngConfigurationError`) | Settings seam only: the SearXNG call still has a fixed 10.0-second per-socket-operation timeout; the configured value becomes a wall-clock budget from provider-bounds US-003 onward | `pipeline/search_providers/searxng.py`'s `searxng_settings_from_config()`, start, even without SearXNG in the chain |
