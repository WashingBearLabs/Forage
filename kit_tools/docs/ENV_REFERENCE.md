...
| `SEARXNG_URL` | none | `http://searxng:8080` | no | no | `retrieval_app.py` line 82, **import** | Default used; assumes a compose service literally named `searxng`. An unreachable SearXNG surfaces as `searxng_unavailable` on `/search`, never as a boot failure. |
...
- **`SEARXNG_URL` is read at import time** (`retrieval_app.py` line 82), not in the lifespan: a change needs a process restart, and a monkeypatch after import does nothing. Its value is also echoed into the `searxng_unavailable` reason on the wire, so never put userinfo in it.
