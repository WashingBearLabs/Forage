...
| `errors` | map | Keyed by `RetrieveErrorCode`: `blocked_domain`, `busy`, `content_too_large`, `fetch_error`, `fetch_timeout`, `invalid_url`, `private_ip`. | Which 422 the callers are hitting; `private_ip` and `invalid_url` are validator refusals, the rest are fetch outcomes. |
