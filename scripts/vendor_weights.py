...
*The constants come from* :mod:`model_fetcher`. ``DEFAULT_MODEL_ID``, the revision pin
...
matters most: each model's manifest entry and mirror tag move **together, in
one commit**, with ``DEFAULT_MODEL_REVISION`` updated for the default model.
``--model-id`` selects that entry explicitly; no model-selection environment
variable is read.
...
def download_snapshot(
    *,
    ...
    (:func:`model_fetcher.acquire_and_load`): the selected model id, the same
...
def generate_manifest(
    ...
    *,
    ...
    """
    document: dict[str, Any] | None = read_manifest_document(manifest_path)
    if document is None and manifest_path.exists():
        raise ManifestGenerationError(
            "the existing manifest is unreadable or invalid; refusing to discard it"
        )
    ...
    if document is None:
        document = {
            "_comment": [
                ...
                "move each model's entry and its mirror tag",
                "ghcr.io/washingbearlabs/forage-weights:<revision> in ONE commit.",
                "For the default model also update "
                "model_fetcher.DEFAULT_MODEL_REVISION.",
                "See docs/weights.md.",
            ],
            "models": {},
        }
    models = cast("dict[str, Any]", document["models"])
    models[model_id] = {"revision": revision, "files": entries}
    return document
...
def manifest_diff(
    ...
    *,
    ...
    if old is None:
        ...
        if not _model_document(old, model_id):
            lines.append(f"model: {model_id} (added)")
        before = _model_document(old, model_id).get("revision")
        after = _model_document(new, model_id).get("revision")
        if before != after:
            lines.append(f"revision: {before!r} -> {after!r}")
        old_files = _files_by_path(old, model_id)

    new_files = _files_by_path(new, model_id)
    ...
    if isinstance(models, dict):
        lines.extend(
            f"{other}: untouched"
            for other in sorted(cast("dict[str, Any]", models))
            if other != model_id
        )
    return tuple(lines)


def _model_document(document: Mapping[str, Any], model_id: str) -> dict[str, Any]:
    models = document.get("models")
    if not isinstance(models, dict):
        return {}
    entry = cast("dict[str, Any]", models).get(model_id)
    return cast("dict[str, Any]", entry) if isinstance(entry, dict) else {}


def _files_by_path(
    document: Mapping[str, Any], model_id: str
) -> dict[str, dict[str, Any]]:
    """Index a manifest's ``files`` list by path, tolerating a malformed one."""
    raw = _model_document(document, model_id).get("files")
...
def extract_and_verify(
    tarball_path: Path, *, manifest_path: Path, model_id: str = DEFAULT_MODEL_ID
) -> VerificationResult:
    ...
    """
    pin = read_manifest_pin(manifest_path, model_id=model_id)
    if pin is None:
        raise VendorError(
...
def mirror_ref(revision: str, *, repository: str = MIRROR_REPOSITORY) -> str:
    ...
    The tag **is** the selected model's revision sha. For the default model it
    also equals ``DEFAULT_MODEL_REVISION``; other models have independent pins.
    """
...
class VendorPlan:
    ...
    def snapshot_dir(self) -> Path:
        ...
        return snapshot_path(self.cache_root, self.model_id, self.revision)
...
def build_plan(args: argparse.Namespace) -> VendorPlan:
    ...
    if revision is None:
        ...
        pin = _model_document(document, model_id) if document is not None else {}
        pinned_revision = pin.get("revision")
        if isinstance(pinned_revision, str):
            revision = pinned_revision
        elif model_id == DEFAULT_MODEL_ID:
            revision = DEFAULT_MODEL_REVISION
        else:
            raise VendorError("the selected model has no pin; supply --revision")
...
def run_plan(plan: VendorPlan, *, environ: Mapping[str, str] | None = None) -> None:
    ...
    if "manifest" in plan.steps:
        ...
        for line in manifest_diff(
            read_manifest_document(plan.manifest_path),
            document,
            model_id=plan.model_id,
        ):
            emit(f"  {line}")
    ...
    if "selfcheck" in plan.steps:
        ...
        result = extract_and_verify(
            plan.tarball_path, manifest_path=plan.manifest_path, model_id=plan.model_id
        )
        if not result.ok:
            raise VendorError(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="The model to vendor (default: the shipped model; no env override)",
    )
    parser.add_argument(
        "--revision",
        default=None,
        help="The upstream commit sha to vendor (default: the selected model's pin)",
    )
    parser.add_argument(
        "--work-dir",
        default=None,
